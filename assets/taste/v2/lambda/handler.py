...
def infer_image_format(image_mime_type: Optional[str], image_b64: Optional[str]) -> str:
    if image_mime_type:
        ...
        if "heic" in image_mime_type or "heif" in image_mime_type:
            return "heic"
    ...
    if image_b64 and image_b64.startswith("data:image/"):
        ...
        if "heic" in prefix or "heif" in prefix:
            return "heic"
...
def normalize_image(image_b64: str, image_format: str):
    """
    Convert unsupported formats (HEIC/HEIF) into JPEG.
...
def build_dish_vector(description: str, image_b64: Optional[str], image_mime_type: Optional[str]):
    ...
    image_b64, image_format = normalize_image(image_b64, image_format)

    image_embedding = normalize(embed_image_base64(image_b64, image_format))

    dish_embedding = normalize(
